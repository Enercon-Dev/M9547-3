...
namespace WinDHCP.Library
{
    public class DhcpServer
    {
        ...
        private const Int32 DhcpMessageMaxSize = 4096;

        private TimeSpan m_OfferTimeout = TimeSpan.FromSeconds(30);
        private TimeSpan m_LeaseDuration = TimeSpan.FromDays(1);
        //private TimeSpan m_LeaseDuration = TimeSpan.FromSeconds(600);

        private NetworkInterface m_DhcpInterface;
        private IPAddress m_DhcpInterfaceAddress;

        private InternetAddress m_StartAddress = new InternetAddress(192, 168, 0, 100);
        private InternetAddress m_EndAddress = new InternetAddress(192, 168, 0, 150);
        private InternetAddress m_Subnet = new InternetAddress(255, 255, 255, 0);
        private InternetAddress m_Gateway = new InternetAddress(192, 168, 0, 1);
        ...
        private System.Threading.Timer m_CleanupTimer;
        ...
        static public BindingList<LogMsg> Log = new BindingList<LogMsg>();
        public BindingList<ListExport> SessionExport = new BindingList<ListExport>();
        //static public ObservableCollection<LogMsg> Log = new ObservableCollection<LogMsg>();
        ...
        public ListExport(AddressLease new_Lease)
            {
                MAC = (new_Lease.Owner == null)? "N/A" : new_Lease.Owner.ToString();
                IP = new_Lease.Address.ToString();
                SesionID = new_Lease.SessionId.ToString();
                Exp = (new_Lease.Expiration == null)? "N/A" : new_Lease.Expiration.ToString();
                ACK = (new_Lease.Acknowledged) ? 1 : 0;
            }

            public static implicit operator ListExport(AddressLease new_Lease)
            {
                return new ListExport( new_Lease);
            }
        ...
        private void UpdateList()
        {
            MainThread.Send(o => {
                SessionExport.Clear();
                m_ActiveLeases.Values.Select(x => (ListExport)x).ToList<ListExport>().ForEach(x=> SessionExport.Add(x));
                },null);
        ...
        public class LogMsg 
        {
            ...
            public static implicit operator LogMsg(string new_Msg)
            {
                return new LogMsg(new_Msg);
            }
        ...
        {
            MainThread.Send(o =>  Log.Insert(0,DateTime.Now.ToLongTimeString()+"  :  "+ msg), null);
        ...
        public void Start(NetworkInterface net_Interface = null)
        {

            MainThread = SynchronizationContext.Current;

            Trace.TraceInformation("Dhcp Server Starting...");
            PrintLog("Dhcp Server Starting...");
            ...
            if (this.m_DhcpInterface == null)
            {
                ...
                PrintLog("Enumerating Network Interfaces.");
                ...
                if (net_Interface != null)
                    m_DhcpInterface = net_Interface;

                Trace.TraceInformation("Using Network Interface \"{0}\".", m_DhcpInterface.Name);
                PrintLog(String.Format("Using Network Interface \"{0}\".", m_DhcpInterface.Name));

                if (m_DhcpInterface != null)
                {
                    byte[] AddrByte = m_DhcpInterface.GetIPProperties().UnicastAddresses.First(x => x.Address.AddressFamily == AddressFamily.InterNetwork).Address.GetAddressBytes();
                    this.m_InactiveLeases.Remove(new InternetAddress(AddrByte));
                    AddressLease lease = new AddressLease(null, new InternetAddress(AddrByte), DateTime.MaxValue);
                    lease.Acknowledged = true;
                    this.m_ActiveLeases.Add(new InternetAddress(AddrByte), lease);
                    UpdateList();
                    AddrByte[3] = 100;
                    m_StartAddress = new InternetAddress(AddrByte);
                    AddrByte[3] = 150;
                    m_EndAddress = new InternetAddress(AddrByte);
                    AddrByte[3] = 1;
                    m_Gateway = new InternetAddress(AddrByte);
                }
            ...
            if (this.m_DhcpInterfaceAddress == null)
            {
               ...
               PrintLog("Unabled to Set Dhcp Interface Address. Check the networkInterface property of your config file.");
                throw new InvalidOperationException("Unabled to Set Dhcp Interface Address.");
            }

            this.m_Abort = false;

            this.m_CleanupTimer = new System.Threading.Timer(new TimerCallback(this.CleanUp), null, 60000, 30000);
            ...
            this.m_DhcpSocket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.IpTimeToLive, 250 );
            //this.m_DhcpInterfaceAddress = IPAddress.Any;
            this.m_DhcpSocket.Bind(new IPEndPoint(this.m_DhcpInterfaceAddress, DhcpPort));
            
            this.Listen();

            this.m_DhcpSocket.SendTo(new byte[10], new IPEndPoint(IPAddress.Broadcast, DhcpClientPort));

            Trace.TraceInformation("Dhcp Service Started.");
           PrintLog("Dhcp Service Started.");
            this.m_DhcpSocket.SendTo(Encoding.ASCII.GetBytes("HELLO!!"),new IPEndPoint(IPAddress.Broadcast, 5060));
        }
        ...
        private void Listen()
        {
            ...
            try
            {
               ...
               PrintLog("Listening For Dhcp Request.");
        ...
        private void OnReceive(IAsyncResult result)
        {
            ...
            if (!this.m_Abort)
            {
               ...
               PrintLog("Dhcp Messages Received, Queued for Processing.");
        ...
        private void CompleteRequest(Object state)
        {
            ...
            if (message.Operation == DhcpOperation.BootRequest)
            {
                ...
                if (messageTypeData != null && messageTypeData.Length == 1)
                {
                    ...
                    switch (messageType)
                    {
                        case DhcpMessageType.Discover:
                           ...
                           PrintLog(String.Format("{0} Dhcp DISCOVER Message Received.", Thread.CurrentThread.ManagedThreadId));
                            this.DhcpDiscover(message);
                            Trace.TraceInformation("{0} Dhcp DISCOVER Message Processed.", Thread.CurrentThread.ManagedThreadId);
                           PrintLog(String.Format("{0} Dhcp DISCOVER Message Processed.", Thread.CurrentThread.ManagedThreadId));
                            break;
                        case DhcpMessageType.Request:
                            Trace.TraceInformation("{0} Dhcp REQUEST Message Received.", Thread.CurrentThread.ManagedThreadId);
                           PrintLog(String.Format("{0} Dhcp REQUEST Message Received.", Thread.CurrentThread.ManagedThreadId));
                            this.DhcpRequest(message);
                            Trace.TraceInformation("{0} Dhcp REQUEST Message Processed.", Thread.CurrentThread.ManagedThreadId);
                           PrintLog(String.Format("{0} Dhcp REQUEST Message Processed.", Thread.CurrentThread.ManagedThreadId));
                            break;
                        default:
                            Trace.TraceWarning("Unknown Dhcp Message ({0}) Received, Ignoring.", messageType.ToString());
                           PrintLog(String.Format("Unknown Dhcp Message ({0}) Received, Ignoring.", messageType.ToString()));
                            break;
                    }
                }
                else
                {
                   ...
                   PrintLog("Unknown Dhcp Data Received, Ignoring.");
                }
            }
        }
        ...
        private void SendOffer(DhcpMessage message, AddressLease offer)
        {
           ...
           PrintLog(String.Format("{0} Sending Dhcp Offer.", Thread.CurrentThread.ManagedThreadId));
           ...
           PrintLog(String.Format("{0} Dhcp Offer Sent.", Thread.CurrentThread.ManagedThreadId));
        }
        ...
        private void SendAck(DhcpMessage message, AddressLease lease)
        {
           ...
           PrintLog(String.Format("{0} Sending Dhcp Acknowledge.", Thread.CurrentThread.ManagedThreadId));
           ...
           PrintLog(String.Format("{0} Dhcp Acknowledge Sent.", Thread.CurrentThread.ManagedThreadId));
        }
        ...
        private void SendNak(DhcpMessage message)
        {
           ...
           PrintLog(String.Format("{0} Sending Dhcp Negative Acknowledge.", Thread.CurrentThread.ManagedThreadId));
           ...
           PrintLog(String.Format("{0} Dhcp Negative Acknowledge Sent.", Thread.CurrentThread.ManagedThreadId));
        }
        ...
        private void SendReply(DhcpMessage response)
        {
            ...
            try
            {
               ...
               //this.m_DhcpSocket.SendTo(response.ToArray(), new IPEndPoint(new IPAddress(response.AssignedAddress), DhcpClientPort));
            }
            catch (Exception ex)
            {
