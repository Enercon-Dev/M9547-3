...
int Arp::handleArpFrame(InFrame& inFrame)
{
  ...
  if (inFrame.readNextIP() != SystemManagement::getPersistentDataCenter().getIPAddr()) {
    return -6;
  }
                        ...
                        bool bAnswer)
{
  ...
  arpMessage.writeIP(SystemManagement::getPersistentDataCenter().getIPAddr());
...
{
  uint32_t myIP = SystemManagement::getPersistentDataCenter().getIPAddr();
  char emptyAddr[6];
